 ...
 */
void acaDP(double *sOpt, double *cOpt, double *objOpt, double *K, int *s, double *G, double *T, double *wFs, int n, int nMa, int m, int k, double *headTail, double *remapedSegConstrains, int nC);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // headTail
    double* headTail = mxGetPr(prhs[6]);
	int nRow = mxGetM(prhs[6]);
    int nCol = mxGetN(prhs[6]);
     
    // remapedSegConstrains
    double* remapedSegConstrains = mxGetPr(prhs[7]);
    int nC = mxGetN(prhs[7]); //nConstraints
    ...
    // dynamic programming
    if (lc == 0) {
        acaDP(sOpt, cOpt, objOpt, K, s, G, T, wFs, n, nMa, m, k, headTail, remapedSegConstrains, nC);
    } else {
...
void acaDP(double *sOpt, double *cOpt, double *objOpt, double *K, int *s, double *G, double *T, double *wFs, int n, int nMa, int m, int k, double *headTail, double *remapedSegConstrains, int nC) {
	...
	for (int v = 0; v < n; ++v) {
		...
		int inTheWindow = 0;//new
		int isTail = 0;//new

		for (int ind = 0; ind < nC; ++ind){
			if (v >= headTail[arrayItem(ind,0,nC)]-1 && v < headTail[arrayItem(ind,1,nC)]-1){ // same as headTail(ind,1) and headTail(ind,2) using macro
				inTheWindow = 1;
			}
			if (v == headTail[arrayItem(ind,1,nC)]-1){
				isTail = 1;
				head = (int)headTail[arrayItem(ind,0,nC)]-1;
				tail = (int)headTail[arrayItem(ind,1,nC)]-1;
			}
		}
		...
		if (isTail){ //if gets in here it is necessarily in a boost mode
			...
			int segInd = 0;
			for (int j = 0; j < m; ++j) {
				segInd = (int)remapedSegConstrains[count-1]-1;
				//q = T[j*m+(segInd-1)];//(int)T[arrayItem(segInd-1, j,m)]
				t2[l[j]] += T[j*m+segInd];
			}
			...
			for(int ind=i; ind<=v; ++ind){
		    	sOpt[ind] = i;      //sOpt(i:v)
				cOpt[ind] = cMi;    //cOpt(i:v)
				objOpt[ind] = dist; //objOpt(i:v)
			}
		}
		//
		else{
			...
			for (int nv = 1; nv <= min(v + 1, nMa); ++nv) {
				...
				if (i == 0 || sOpt[i - 1] >= 0) {
					double dist;
					if (i == 0 || i - 1 == tail) {//if stating a new segment after a known segment
						dist = dMi;
					} else {
						dist = objOpt[i - 1] + dMi;
					}
